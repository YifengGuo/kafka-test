        ...
        try (KafkaConsumer<String, String> consumer = new KafkaConsumer<>(properties)) {  // close consumer in try
            ...
            consumer.subscribe(Arrays.asList(TOPIC), new ConsumerRebalanceListener() {
                ...
                public void onPartitionsRevoked(Collection<TopicPartition> partitions) {
                    // sync commit before rebalance can avoid duplicate data consumption after rebalance
                    // if partitions re-assigned to other consumers but last poll() was not committed
